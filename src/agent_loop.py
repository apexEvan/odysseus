...
Streaming agent loop for Odysseus.
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "list_cached_models": "- ```list_cached_models``` — List models already on disk. Args (JSON, all optional): {\"host\": \"workstation or user@gpu-server\"?, \"model_dir\": \"/data/models,/extra\"?}. Friendly Cookbook server names work. Use for 'what models do I have' / 'show cached models' / 'is X downloaded'.",
    "app_api": """\
```app_api
